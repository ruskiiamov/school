...
func TestStudentMarksSummary(t *testing.T) {
	...
	assert.Contains(t, body, `href="`+html.EscapeString(diaryURL("/diary", f.today, url.Values{"lesson": {idValue(algebra)}}))+`" title="`+f.today.Format("02.01.2006")+" · Ответ на уроке · у доски"+`"`)
...
func TestParentAndAdminMarksSummary(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, code)
	assert.Contains(t, body, `href="/admin/diary/`+idValue(f.student)+`/summary?q=%D0%B8%D0%B2%D0%B0%D0%BD%D0%BE%D0%B2%D0%B0"`)
	...
	assert.Contains(t, body, `href="`+html.EscapeString(diaryURL("/admin/diary/"+idValue(f.student), f.today, url.Values{"lesson": {idValue(algebra)}, "q": {"иванова"}}))+`"`)
	assert.Contains(t, body, ">5</a>")
