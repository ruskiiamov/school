...
func TestClassAssignmentsAssignReplaceRemove(t *testing.T) {
	...
	card := get(t, env.handler, classPathFor(class, ""), admin).Body.String()
	...
	assertRedirect(t, postForm(t, env.handler, classPathFor(class, "/assignments"), assignForm(algebra, anna), []*http.Cookie{admin}, nil), classPathFor(class, ""))
	...
	assertRedirect(t, postForm(t, env.handler, classPathFor(class, "/assignments"), assignForm(algebra, viktor), []*http.Cookie{admin}, nil), classPathFor(class, ""))
	...
	assertRedirect(t, postForm(t, env.handler, classPathFor(class, "/assignments"), assignForm(history, anna), []*http.Cookie{admin}, nil), classPathFor(class, ""))
	...
	for name, tc := range cases {
		rejected := postForm(t, env.handler, classPathFor(class, "/assignments"), tc.form, []*http.Cookie{admin}, nil)
	...
	blank := postForm(t, env.handler, classPathFor(class, "/assignments"), url.Values{}, []*http.Cookie{admin}, nil).Body.String()
	...
	assert.Equal(t, http.StatusNotFound, postForm(t, env.handler, classPathFor(other, "/assignments/"+strconv.FormatInt(first, 10)+"/remove"), nil, []*http.Cookie{admin}, nil).Code)
	assertRedirect(t, postForm(t, env.handler, remove, nil, []*http.Cookie{admin}, nil), classPathFor(class, ""))
	assert.Equal(t, http.StatusNotFound, postForm(t, env.handler, remove, nil, []*http.Cookie{admin}, nil).Code)
	assert.Equal(t, http.StatusNotFound, postForm(t, env.handler, classPathFor(class, "/assignments/abc/remove"), nil, []*http.Cookie{admin}, nil).Code)
...
func TestClassAssignmentsHtmxFragment(t *testing.T) {
	...
	assigned := postForm(t, env.handler, classPathFor(class, "/assignments"), assignForm(algebra, anna), []*http.Cookie{admin}, htmx)
	...
	rejected := postForm(t, env.handler, classPathFor(class, "/assignments"), assignForm(algebra, 0), []*http.Cookie{admin}, htmx)
