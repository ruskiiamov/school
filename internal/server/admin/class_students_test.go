...
func TestClassStudentsAddAndRemove(t *testing.T) {
	...
	servertest.AssertRedirect(t, servertest.PostForm(t, env.Handler, classPathFor(classA, "/students"), studentForm(free), []*http.Cookie{admin}, nil), classPathFor(classA, ""))
	...
	students := servertest.Get(t, env.Handler, "/admin/students?class="+strconv.FormatInt(classA, 10), admin)
	assert.Contains(t, students.Body.String(), "Кузнецов Олег")
	assert.NotContains(t, students.Body.String(), "Петров Иван")

	taken := servertest.PostForm(t, env.Handler, classPathFor(classA, "/students"), studentForm(inB), []*http.Cookie{admin}, nil)
	...
	for name, form := range map[string]url.Values{
		...
		rejected := servertest.PostForm(t, env.Handler, classPathFor(classA, "/students"), form, []*http.Cookie{admin}, nil)
	...
	servertest.AssertRedirect(t, servertest.PostForm(t, env.Handler, remove, nil, []*http.Cookie{admin}, nil), classPathFor(classA, ""))
	...
	assert.Equal(t, http.StatusNotFound, servertest.PostForm(t, env.Handler, classPathFor(classA, "/students/abc/remove"), nil, []*http.Cookie{admin}, nil).Code)
	assert.Equal(t, http.StatusNotFound, servertest.PostForm(t, env.Handler, classPathFor(999, "/students"), studentForm(free), []*http.Cookie{admin}, nil).Code)
}
...
func TestClassStudentsHtmxFragment(t *testing.T) {
	...
	taken := servertest.PostForm(t, env.Handler, classPathFor(classA, "/students"), studentForm(inB), []*http.Cookie{admin}, htmx)
	assert.Equal(t, http.StatusOK, taken.Code)
	assert.NotContains(t, taken.Body.String(), "<html")
	assert.Contains(t, taken.Body.String(), `id="class-students"`)
	assert.Contains(t, taken.Body.String(), "Ученик уже в классе 7Б")

	removed := servertest.PostForm(t, env.Handler, classPathFor(classA, "/students/"+strconv.FormatInt(free, 10)+"/remove"), nil, []*http.Cookie{admin}, htmx)
...
func TestClassStudentsReadOnlyOutsideCurrentYear(t *testing.T) {
	...
	for _, id := range []int64{past, closed} {
		...
		forged := servertest.PostForm(t, env.Handler, classPathFor(id, "/students"), studentForm(student), []*http.Cookie{admin}, nil)
		assert.Equal(t, http.StatusOK, forged.Code)
		assert.Contains(t, forged.Body.String(), "Такого класса нет в текущем году")

		removed := servertest.PostForm(t, env.Handler, classPathFor(id, "/students/"+strconv.FormatInt(student, 10)+"/remove"), nil, []*http.Cookie{admin}, nil)
		assert.Equal(t, http.StatusOK, removed.Code)
		assert.Contains(t, removed.Body.String(), "Такого класса нет в текущем году")
	}

	pastCard := servertest.Get(t, env.Handler, classPathFor(past, ""), admin).Body.String()
...
func TestClassCardActionsHiddenFromOtherRoles(t *testing.T) {
	...
	for _, path := range []string{
		...
		servertest.AssertRedirect(t, servertest.PostForm(t, env.Handler, path, studentForm(student), nil, nil), "/login")
		assert.Equal(t, http.StatusNotFound, servertest.PostForm(t, env.Handler, path, studentForm(student), []*http.Cookie{teacher}, nil).Code, path)
	}
