...
func (r *SessionRepo) ByID(ctx context.Context, id string) (Session, User, error) {
	const query = `SELECT s.id, s.user_id, s.created_at, s.expires_at,
		u.id, u.login, u.password_hash, u.full_name, u.role, u.created_at, u.updated_at
	...
	err := r.db.QueryRowContext(ctx, query, id).Scan(
		...
		&user.ID, &user.Login, &user.PasswordHash, &user.FullName, &user.Role, &userCreatedAt, &userUpdatedAt,
	)
