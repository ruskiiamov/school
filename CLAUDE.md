...
«Отсутствовал» (сохраняет сразу) и комментарий за `<details>` без JS,
обе формы несут скрытым полем текущее значение друг друга;
...
`app.js` перехватывает `htmx:confirm`), `data-state` у сайдбара. Обработчики
